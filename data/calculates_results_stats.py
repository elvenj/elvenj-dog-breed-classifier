...
def calculates_results_stats(results_dic):
    ...
    # Percorre o dicionário dos resultados para computar contagens
    for key in results_dic:
        ...
        # Conta imagens de cães e não cães
        if is_pet_dog == 1:
            results_stats_dic['n_dogs_img'] += 1
            # Conta acertos corretos de cães
            if is_classifier_dog == 1:
                results_stats_dic['n_correct_dogs'] += 1
            # Conta acertos da raça corretamente identificada (match e dog)
            if match == 1:
                results_stats_dic['n_correct_breed'] += 1
        else:
            results_stats_dic['n_notdogs_img'] += 1
            # Conta acertos para não cães
            if is_classifier_dog == 0:
                results_stats_dic['n_correct_notdogs'] += 1

        # Conta todos os matches entre pet e classifier
        if match == 1:
            results_stats_dic['n_match'] += 1

    # Calcula as porcentagens, protegendo contra divisão por zero
